...
namespace Microsoft.Azure.Commands.Network.Bastion.SessionManagement
{
    using System;
    ...
    [Cmdlet(VerbsCommon.Remove,
        ...
        OutputType(typeof(List<PSBastionSessionState>))]
    public class RemoveAzBastionActiveSessionCommand : BastionBaseCmdlet
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void Execute()
        {
            ...
            if (disconnectSessionsIterable != null)
            {
                foreach (BastionSessionState disconnectedSessionState in disconnectSessionsIterable)
                {
                    ...
                    psSessionStateList.Add(psSessionState);
                }
            }

            WriteVerbose($"Found {psSessionStateList.Count} session states");
            WriteObject(psSessionStateList);
        }
    }
}
